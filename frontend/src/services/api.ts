...
import { ExecutionResult, SubmissionResult, CodeExecutionRequest, SubmissionRequest } from '../types/api'
...
export const getSubmission = async (submissionId: string): Promise<SubmissionResult> => {
  const response = await api.get<SubmissionResult>(`/code/submissions/${submissionId}`)
  return response.data
}
